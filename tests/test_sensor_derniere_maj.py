...
The sensor used to fall back to ``coordinator.last_update_success_time``, an attribute Home Assistant's DataUpdateCoordinator does not define. Guarded by a ``hasattr`` check, the fallback silently never fired, so the sensor reported ``unknown`` whenever the API carried no ``datePublication`` — that is, in normal operation, outside an outage.
"""
...
def maj_for(payload: dict[str, Any], last_success_time=None):
    ...
    The sensor is built with ``__new__`` to bypass the Home Assistant entity constructor; ``native_value`` only reads the coordinator.
    """
