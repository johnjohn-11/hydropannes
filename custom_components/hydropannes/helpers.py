...
class HydroPannesHelperMixin:
    ...
    def _is_reprise_graduelle(self, intr: dict[str, Any] | None = None) -> bool:
        ...
        ``repriseGraduellePossible`` is consumed from the payload root, but it is also listed among the interruption-level fields the integration knows about. Both locations are honoured so the flag is not missed if Hydro-Québec reports it per interruption rather than at the root.
        """
