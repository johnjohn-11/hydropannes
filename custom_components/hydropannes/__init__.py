...
async def async_migrate_entry(hass: HomeAssistant, entry: HydroPannesConfigEntry) -> bool:
    ...
    Version 1 stored the location name in ``entry.data`` next to the entry title, kept in step by a dedicated options flow. The title is now the only source of truth — it is what Home Assistant's own Rename action updates — so the stored copy is dropped; leaving it would let the two drift apart.
    """
    if entry.version == 1:
...
async def _async_update_listener(hass: HomeAssistant, entry: HydroPannesConfigEntry) -> None:
    ...
    A rename changes the entry title, which names the device and prefixes every entity name, so the entry is reloaded for the new title to take effect without restarting Home Assistant.
    """
